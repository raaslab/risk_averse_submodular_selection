...
# r_node = pos_node[0:R]
r_node[0] = ox.get_nearest_node(G, (37.2405, -80.4194))
r_node[1] = ox.get_nearest_node(G, (37.214, -80.4004))
r_node[2] = ox.get_nearest_node(G, (37.2074, -80.4207))
d_node = ox.get_nearest_node(G, (37.227, -80.413))
print(r_node)
print(d_node)
...
for i in range(R):
	...
	#find the route between these nodes then plot it
		route = nx.shortest_path(G, r_node[i], d_node, weight='length')
		# route_degree
		routes_degree[j][i] = sum([avg_weighted_neighbor_degree[node] for node in route])
		routes.append(route)
		# print(route)
		# how long is our route in kiometers?
		routes_length[j][i] = nx.shortest_path_length(G, r_node[i], d_node, weight='length')/1000
		print(routes_length)
		print(routes_degree)
