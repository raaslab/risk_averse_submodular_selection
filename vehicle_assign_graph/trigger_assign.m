...
% after we know goalroutes_length and goalroutes_degree
% we can sample on the efficiency distribution from each node to each
% demand locations (Actually, each demand to each node.)
...
% first calculate the routes_length and routes_degree for each d_node to
% each r_node
routes_length = zeros(D,R);
routes_degree = zeros(D,R);
% routes_effi_samp =zeros(1, n_samp, D, R);
% create a cell_table to update the edge the vehicle is on
% first value should be the first edge from each vehicle to each demand on
% the route
ve_edge_de = cell(D, R);
...
%         routes_effi_samp(:,:,i,j) = goalroutes_effi_samp(:,:,i,r_inx);
        ...
        ve_edge_de{i,j} = [0, len, degree]; % first element is to store the virtual node   
        % store reach_state, current_len, degree, sample
        ve_edge_de_last{i,j} = [0, len, degree, 0]; % first element is to store the flag of reaching new node or not. 0, new. 1, old
    end
end
%%
% %%% conduct online assignment, the terminated condition is
% %%% all the demands are served.
% 1, presample 
n_samp = 1000;
...
online_assign_inx = zeros(1,max_num_node);
...
%     [d_nodes_act, r_nodes_act, r_nodes_full, ve_edge_de, ve_edge_de_last,...
%         routes_length_temp, routes_degree_temp, greset_full, greset_update, ...
%         tri_flag, reach_next_time]...
%         = update_parameter_trigger_minnext(d_nodes_act, r_nodes_act, r_nodes_full, ...
%         cvar_greset, greset_full, greset_update, allnodes, goalroutes, ...
%         goalroutes_length, goalroutes_degree,...
%         ve_edge_de, ve_edge_de_last, edges_length_degree, tri_flag, tri_ratio); 
    ...
    [d_nodes_act, r_nodes_act, r_nodes_full, ve_edge_de, ve_edge_de_last,...
        routes_length_temp, routes_degree_temp, greset_full, greset_update, ...
        tri_flag, reach_next_time]...
        = update_parameter_trigger_minnext2(d_nodes_act, r_nodes_act, r_nodes_full, ...
        cvar_greset, greset_full, greset_update, allnodes, goalroutes, ...
        goalroutes_length, goalroutes_degree,...
        ve_edge_de, ve_edge_de_last, edges_length_degree, tri_flag, tri_ratio, reach_next_time);            

    % store assignment for each move
    online_assign{cnt} = greset_full; 
    online_dnodes{cnt} = d_nodes;
    online_rnodes{cnt} = r_nodes;
    online_edge{cnt} = ve_edge_de_last;
    cnt = cnt + 1;
    arrive_time = arrive_time + reach_next_time;
    % plot  
    graph_plot(greset_full, allnodes, r_nodes_full, goalroutes);        
%     
end
% plot(online_assign_inx); hold on
idx = find( online_assign_inx == 1 );
figure; hAxes = gca;
stem( hAxes, idx, ones(numel(idx)) );
% store online assign for each 
online_assign = online_assign(~cellfun(@isempty, online_assign));
cvar_greset_risks{i} = online_assign;
