...
function [d_nodes_1, r_nodes_1, rnodes_full_1, ve_edge_de_1, ve_edge_de_last_1,...
    routes_length, routes_degree, greset_full_1, greset_update_1, tri_flag, min_reach_next_time_1]...
            = update_parameter_trigger_minnext2(d_nodes, r_nodes, rnodes_full, cvar_set, ...
            greset_full, greset_update, allnodes, goalroutes, goalroutes_length, ...
            goalroutes_degree, ve_edge_de, ve_edge_de_last, edges_length_degree, tri_flag, tri_ratio, ...
            min_reach_next_time)
        ...
        for i = 1 : length(d_nodes(:,1))
            for j = 1 : length(cvar_set{i})
                ...
                if ve_edge_de_last_1{d_nodes(i,1), r_nodes(cvar_set{i}(j),1)}(1) == 0
                    % truncated gaussian distribution range [v1v2_len, inf]
                    % travel time is proportional to travel length with uncertainty scaling
                    s_ini = trandn(0,1); %v1v2_len/sqrt(v1v2_degree)
                    s = v1v2_len + s_ini * sqrt(v1v2_degree);
                else %update the old sample time
                    s = ve_edge_de_last_1{d_nodes(i,1), r_nodes(cvar_set{i}(j),1)}(4) - min_reach_next_time_1; 
                end
                %update the time to reach the next node, stay on the current node/edge or move to the next edge
                ve_edge_de_last_1{d_nodes(i,1), r_nodes(cvar_set{i}(j),1)} = [0, v1v2_len, v1v2_degree, s];
        ...
        % d_node, r_node are always the reference
         ...
         for i = 1 : length(d_nodes(:,1)) % note that set and d_node have the same length
             ...
             for j = 1 : length(cvar_set{i}) %check its elements
                 ...
                 greset_full_1{d_inx} = [greset_full_1{d_inx}, ...
                     r_nodes(cvar_set{i}(j), 1)];
...
%                      = [0, v1v2_len, v1v2_degree, s];
                 ...
                 if reach_next_time{i}(j) == min_reach_next_time_1 || ...
                         ismember(r_nodes(cvar_set{i}(j), 2), ...
                         [3586727936, 3586727937, 3586727938, ...
        ...
        for i = 1 : length(d_nodes_1(:,1)) 
            ...
            % if not the last demand and
            % if the node has more than one vehicles assigned to the demand 
            if length(d_nodes_1(:,1))>1 && length(greset_update_1{d_inx})> 1 
                % find the minimum length and maximum length 
                % and their  indices for demand inx
                [min_len, min_j] = min(routes_length(i,:));
                [max_len, max_j] = max(routes_length(i,:));
                % if the min length is less than half of max_len 
                % and its uncertainty is smaller
                % the max_len may need to assign to other demand
                if min_len <= tri_ratio * max_len ...
                        && routes_degree(i, min_j) <= routes_degree(i, max_j)
                    tri_flag = 1;
                    % disp('trigger');
                end
            end
